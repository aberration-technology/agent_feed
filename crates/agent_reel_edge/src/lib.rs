use agent_reel_directory::{
    DirectoryError, DirectoryStore, GithubDiscoveryTicket, GithubProfileView, RemoteUserRoute,
    SignedBrowserSeed,
};
use agent_reel_identity::GithubLogin;
use agent_reel_identity_github::{
    GithubResolveError, GithubResolver, GithubUserResponse, StaticGithubResolver,
};
use agent_reel_p2p_proto::{github_provider_key, github_user_topic};
...
impl EdgeConfig {
    ...
    pub fn mainnet() -> Self {
        Self {
            network_id: "agent-reel-mainnet".to_string(),
...
#[derive(Clone, Debug)]
pub struct EdgeResolver<R = StaticGithubResolver> {
    pub config: EdgeConfig,
    pub github: R,
    pub directory: DirectoryStore,
}

impl<R: GithubResolver> EdgeResolver<R> {
    #[must_use]
    ...
    pub fn resolve_github_user(
        ...
        let ticket = GithubDiscoveryTicket {
            ...
            signature: agent_reel_p2p_proto::Signature::unsigned(),
        }
...
async fn auth_github(
    ...
    if !matches!(
        client.as_str(),
        "feed-browser" | "feed-cli" | "agent-reel-browser" | "agent-reel-cli"
    ) {
...
fn fetch_github_profile(
    access_token: &str,
) -> Result<agent_reel_identity_github::GithubProfile, String> {
    let output = Command::new("curl")
        .args([
            "-fsSL",
            "-H",
...
impl GithubResolver for CurlGithubResolver {
    fn resolve_login(
        &self,
        login: &GithubLogin,
    ) -> Result<agent_reel_identity_github::GithubProfile, GithubResolveError> {
...
mod tests {
    ...
    use agent_reel_identity_github::GithubProfile;
    use agent_reel_p2p_proto::FeedVisibility;
