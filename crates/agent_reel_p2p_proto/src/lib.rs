...
pub fn directory_topic(network_id: &str) -> String {
    format!("agent-reel.v1.{network_id}.directory")
}

#[must_use]
pub fn presence_topic(network_id: &str) -> String {
    format!("agent-reel.v1.{network_id}.presence")
}

#[must_use]
pub fn github_user_topic(network_id: &str, github_user_id: u64) -> String {
    format!(
        "agent-reel.v1.{network_id}.github.{}",
...
mod tests {
    ...
    fn feed_topic_hides_raw_feed_id() {
        let topic = feed_topic("agent-reel-mainnet", "github:mosure/workstation");
        assert!(topic.starts_with("agent-reel.v1.agent-reel-mainnet.feed."));
    ...
    fn github_user_topic_hides_login_and_numeric_id() {
        let topic = github_user_topic("agent-reel-mainnet", 123_456);
        assert!(topic.starts_with("agent-reel.v1.agent-reel-mainnet.github."));
        assert!(!topic.contains("mosure"));
